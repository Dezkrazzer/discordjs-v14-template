    ...
    constructor(client) {
        super(client, {
            ...
            slash: new Discord.SlashCommandBuilder()
                .setName("help")
                .setDescription("Displays a list of available commands or detailed information about a specific command.")
    ...
    async execute(ctx) {
        ...
        if (!targetCommand) {
            ...
            for (const category of categories) {
                const commands = ctx.client.commands.filter((cmd) => cmd.category === category).map((cmd) => `\`${cmd.name}\``).join(", ");
            ...
            const embed = new Discord.EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: "Cooldown", value: `\`${command.cooldown} sec\``, inline: true },
                    { name: "Dev Only", value: `\`${command.devOnly ? "Yes" : "No"}\``, inline: true },
                    { name: "Description", value: `> ${command.description}`, inline: false },
                    { name: "Aliases", value: command.aliases.length > 0 ? `> ${command.aliases.map(alias => `\`${alias}\``).join(", ")}` : "> None", inline: false },
                    { name: "Usage", value: `${command.usage ? `\`\`\`${ctx.prefix}${command.name} ${command.usage}\`\`\`` : "No usage information available."}`, inline: false },
                )
                .setFooter({
